...
def assign_number_to_sequence(seq,scheme):
    if scheme=='aho':
        abchain = abnumber.Chain(seq, scheme='aho',cdr_definition="chothia")
    if scheme=='imgt':
        abchain = abnumber.Chain(seq, scheme='imgt')
...
def renumber(in_pdb, out_pdb,scheme,return_other_chains=False):
    ...
    for chain in model:
        try:
            ...
            if abchain.chain_type == 'H':
                ...
                print('chain_type',abchain.chain_type)
...
def main():
    ...
    parser.add_argument('-i','--in_pdb', type=str,help='input antibody/antigen_antibody pdb file path')
    parser.add_argument('-o','--out_pdb', type=str,help='output pdb file path')
    parser.add_argument('-s','--scheme', type=str,help='scheme:aho or imgt')
    args = parser.parse_args()



    renumber(args.in_pdb, args.out_pdb,args.scheme)
